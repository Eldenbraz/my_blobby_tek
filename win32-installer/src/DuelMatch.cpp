...
void DuelMatch::step()
{
	...
	for( const auto& event : mEvents )
	{
		...
		case MatchEvent::BALL_HIT_NET:
			mPhysicWorld->setBallVelocity( mPhysicWorld->getBallVelocity().scale(1.2) );
...
{
	if (side == LEFT_PLAYER)
		mPhysicWorld->setBallPosition( Vector2(rand() % ((300 - 100) + 1) + 100, STANDARD_BALL_HEIGHT) );
	else if (side == RIGHT_PLAYER)
		mPhysicWorld->setBallPosition( Vector2(rand() % ((700 - 500) + 1) + 700, STANDARD_BALL_HEIGHT) );
	else
